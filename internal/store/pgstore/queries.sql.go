...
const markMessagesAsAnswered = `-- name: MarkMessagesAsAnswered :exec
UPDATE messages
SET
    answered = true
WHERE
...
func (q *Queries) MarkMessagesAsAnswered(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markMessagesAsAnswered, id)
	return err
}
